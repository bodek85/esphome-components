...
// C1 usually uses the B format for the data link layer, less CRCs = less
// overhead.

// The im871a can for example receive C1a, but it is unclear if there are any
// meters that use it.
...
enum class CI_TYPE { ELL, NWL, AFL, TPL };

enum class TPL_LENGTH { NONE, SHORT, LONG };
...
enum class FrameType { WMBUS, MBUS, HAN };
...
struct AboutTelegram {
  ...
  std::string device;
  // The device's opinion of the rssi, best effort conversion into the dbm
  // scale. -100 dbm = 0.1 pico Watt to -20 dbm = 10 micro W Measurements
  // smaller than -100 and larger than -10 are unlikely.
  int rssi_dbm{};
  // WMBus or MBus
  FrameType type{};
...
// Mark understood bytes as either PROTOCOL, ie dif vif, acc and other header
// bytes. Or CONTENT, ie the value fields found inside the transport layer.
enum class KindOfData { PROTOCOL, CONTENT };

// Content can be not understood at all NONE, partially understood PARTIAL when
// typically bitsets have been partially decoded, or FULL when the volume or
// energy field is by itself complete. Encrypted if it yet decrypted. Compressed
// and no format signature is known.
...
public:
  ...
  // the first is the dll_id_mvt, ell_id_mvt, nwl_id_mvt, and the last is the
  // tpl_id_mvt.
  ...
  uchar dll_id_b[4]{};       // 4 bytes, address in BCD = 8 decimal
                             // 00000000...99999999 digits.
  ...
  int suffix_size{}; // Size of suffix after the APL content. Usually empty, but
                     // can be MACs!
  int mfct_0f_index = -1; // -1 if not found, else index of the 0f byte, if
                          // found, inside the difvif data after the header.
  int mfct_1f_index =
      -1; // -1 if not found, else index of the 1f byte, if found, then there
          // are more records in the next telegram.
  int force_mfct_index = -1; // Force all data after this offset to be mfct
                             // specific. Used for meters not using 0f.
  void extractFrame(std::vector<uchar> *fr);   // Extract to full frame.
  void extractPayload(std::vector<uchar> *pl); // Extract frame data containing
                                               // the measurements, after the
                                               // header and not the suffix.
  void extractMfctData(
      std::vector<uchar> *pl); // Extract frame data after the DIF 0x0F.
  ...
  void addExplanationAndIncrementPos(std::vector<uchar>::iterator &pos, int len,
                                     KindOfData k, Understanding u,
                                     const char *fmt, ...);
  void setExplanation(std::vector<uchar>::iterator &pos, int len, KindOfData k,
...
private:
  ...
  bool checkMAC(std::vector<uchar> &frame, std::vector<uchar>::iterator from,
                std::vector<uchar>::iterator to, std::vector<uchar> &mac,
...
// Decode only the standard defined bits in the tpl status byte. Ignore the top
// 3 bits. Return "OK" if sts == 0
std::string decodeTPLStatusByteOnlyStandardBits(uchar sts);
// Decode the standard bits and report the top 3 bits if set as for example:
// UNKNOWN_0x80 Return "OK" if sts == 0
