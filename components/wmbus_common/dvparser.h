...
void extractDV(DifVifKey &s, uchar *dif, int *vif, bool *has_difes,
               bool *has_vifes);
...
struct DVEntry {
  ...
  DVEntry(int off, DifVifKey dvk, MeasurementType mt, Vif vi,
      ...
      : offset(off), dif_vif_key(dvk), measurement_type(mt), vif(vi),
        ...
        tariff_nr(ta), subunit_nr(su), value(val) {}

  DVEntry()
      : offset(999999), dif_vif_key("????"),
        measurement_type(MeasurementType::Instantaneous), vif(0), storage_nr(0),
        tariff_nr(0), subunit_nr(0), value("x") {}
...
struct FieldMatcher {
  ...
  // and you want to catch the second matching entry, then set the index nr
...
// find an existing difvif entry in the values based on the desired value
// information type. Like: Volume, VolumeFlow, FlowTemperature,
// ExternalTemperature etc in combination with the storagenr. (Later I will add
// tariff/subunit)
